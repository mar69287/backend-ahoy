...
function addRoutes(app, evm) {
	...
	app
	 ...
	 .post(async (req, res) => {
	   ...
	   try {
		...
		const parsedTotalPrice = ethers.parseUnits(totalPrice.toString(), 6);
		const parsedSecurityDeposit = ethers.parseUnits(securityDeposit.toString(), 6);
		console.log({parsedTotalPrice})
		console.log({parsedSecurityDeposit})
    ...
    //   const apiPrompt = `Retro, 8-bit, Grand, Impressive, 8-bit, Closeup on boat, Natural Background, Amazing`;
      const apiPrompt = `${imageUrl} Retro, 8-bit, Closeup on boat, Amazing and Epic, Natural Background, pixelated, very detailed --stylize 500 --style raw --v 6.0 --sref https://s.mj.run/moYAVD4tWRE --sw 200`;
